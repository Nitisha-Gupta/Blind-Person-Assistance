...
function App() {
  ...
  const handleObjectDetection = async () => {
    if (isDetectingObjects) {
      ...
      setIsLoading(true);
      try {
        const response = await fetch(`${BACKEND_URL}/api/stop-object-detection`, {
          method: 'POST',
        });
        setIsDetectingObjects(false);
        setMode(null);
      } catch (err) {
        console.error('Error stopping object detection:', err);
        alert('Failed to stop object detection.');
      } finally {
        setIsLoading(false);
      }
    } else {
      ...
      setIsLoading(true);
      try {
        const response = await fetch(`${BACKEND_URL}/api/detect-object`, {
          method: 'POST',
        });
        ...
        alert('Failed to start object detection.');
      } finally {
        setIsLoading(false);
      }
    }
  ...
  const handleEmergency = async () => {
    setIsLoading(true);
    try {
      const response = await fetch(`${BACKEND_URL}/api/voice-command-sos`, {
        method: 'POST',
      });
      const data = await response.json();
      // Handle emergency response if needed
    } catch (error) {
